...
echo
echo "Setting up the JBoss Six Products Demo ${DEMO} environment..."
echo

# make some checks first before proceeding.
if [[ -f $SRC_DIR/$EAP || -L $SRC_DIR/$EAP ]]; then
...
echo " - adding rh6pd car inurance model jar to business central admin console classpath..."
echo
cp support/rh6pd-businessRulesModel.jar $SERVER_DIR/business-central-server.war/WEB-INF/lib

echo " - adding rh6pd javascript war to servers deploy directory..."
echo
cp support/rh6pd-webJsInterface-1.0.0-SNAPSHOT.war $SERVER_DIR/rh6pd-webJsInterface-1.0.0-SNAPSHOT.war
