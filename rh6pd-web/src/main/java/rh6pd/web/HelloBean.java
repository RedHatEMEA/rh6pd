...
public class HelloBean implements Serializable {
	...
	public String getSayWelcome() {
		...
		if ("".equals(name) || name == null) {
			...
			if (motors.contains(person.getCar())) {
				System.out.println("Motors Contains: "
						+ person.getCar().getMake().toString());
			} else {
			...
			return "Details successfully submited to brms: " + name + " : "
					+ age + " : " + make + " : " + model
	...
	protected static KnowledgeBase readKnowledgeBase() throws Exception {
		KnowledgeBuilder kbuilder = KnowledgeBuilderFactory
				.newKnowledgeBuilder();
		kbuilder.add(
				ResourceFactory.newClassPathResource("CalculatePremium.drl"),
				ResourceType.DRL);
